...
class MisSiir:
    ...
    def _login(self, driver: webdriver.Chrome):
        driver.get(self.url)
        driver.find_element(
            By.XPATH, "//input[@id='rutcntr']").send_keys(self.rut)
        driver.find_element(
            By.XPATH, "//input[@id='clave']").send_keys(self.tax_code)
    ...
    def _misii_doc(driver: webdriver.Chrome, doc: Document):
        try:
            ...
            email = driver.find_element(
                By.XPATH, "//p[@id='mailCntrNoti']").text
    ...
    def _inicio_de_actividades_y_termino_de_giro(driver: webdriver.Chrome, doc: Document):
        try:
            click_ele = driver.find_element(
                By.XPATH, "//*[@id=\"headingTwo\"]/h4/a/div/div/div")
            ...
            html = driver.find_element(
                By.XPATH, table_xpath).get_attribute('innerHTML')
            soup = BeautifulSoup(html, 'html.parser')
            td_elements = soup.find_all('td')
            td_values = [td.get_text(strip=True) for td in td_elements]
            sub_lists = [td_values[i:i + 3]
            ...
            headers = ['Fecha constitución',
                       'Inicio actividades', 'Término giro']
            ...
            logging.info(
                '--> Inicio de actividades y término de giro extracted.')
    ...
    def _representantes_legales_vigentes(driver: webdriver.Chrome, doc: Document):
        try:
            click_ele = driver.find_element(
                By.XPATH, "//div[@id='headingConsultas']//div[@class='ic_arrow']")
            driver.execute_script("arguments[0].click();", click_ele)
    ...
    def _socios_y_capital(driver: webdriver.Chrome, doc: Document):
        try:
            click_ele = driver.find_element(
                By.XPATH, "//div[@id='headingActualizacion']")
            driver.execute_script("arguments[0].click();", click_ele)
            ...
            # Filas de contenido
            for ind, line in enumerate(sub_lists):
                # Se ignora el índice 7 (columna 8), ya que no está en el diseño
                for col in range(7):
                    cell = table.cell(ind + 1, col)
                    cell.text = line[col]
                    cell.paragraphs[0].alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
    ...
    def _actividades_economicas(driver: webdriver.Chrome, doc: Document):
        try:
            click_ele = driver.find_element(
                By.XPATH, "//div[@id='headingP10']//div[@class='ic_arrow']")
            driver.execute_script("arguments[0].click();", click_ele)
            ...
            td_elements = soup.find_all('td')
            td_values = [td.get_text(strip=True) for td in td_elements]
            sub_lists = [td_values[i:i + 5]
                         for i in range(0, len(td_values), 5)]
            ...
            headers = ['Actividad', 'Código',
                       'Categoría tributaria', 'Afecta IVA', 'A partir de']
    ...
    def _sociedades_a_las_que_pertenece_el_contribuyente(driver: webdriver.Chrome, doc: Document):
        try:
            click_ele = driver.find_element(
                By.XPATH, "//div[@id='headingP11']//div[@class='box_arrow']")
            driver.execute_script("arguments[0].click();", click_ele)
            ...
            td_elements = soup.find_all('td')
            td_values = [td.get_text(strip=True) for td in td_elements]
            sub_lists = [td_values[i:i + 9]
                         for i in range(0, len(td_values), 9)]

            doc.add_heading(
                'Sociedades a las que pertenece el contribuyente', 1)
            ...
            logging.info(
                '--> Sociedades a las que pertenece el contribuyente extracted.')
    ...
    def _caracteristicas_del_contribuyente(driver: webdriver.Chrome, doc: Document):
        try:
            click_ele = driver.find_element(
                By.XPATH, "//div[@id='ctracc_9']//div[@class='box_arrow']")
            driver.execute_script("arguments[0].click();", click_ele)
            ...
            td_elements = soup.find_all('td')
            td_values = [td.get_text(strip=True) for td in td_elements]
            sub_lists = [td_values[i:i + 2]
                         for i in range(0, len(td_values), 2)]
            ...
            logging.error(
                f"Error extrayendo características del contribuyente: {e}")

    @staticmethod
    def _bienes_raices(driver: webdriver.Chrome, doc: Document):
        try:
            click_ele = driver.find_element(
                By.XPATH, "//div[@id='headingP14']//div[@class='box_arrow']")
            driver.execute_script("arguments[0].click();", click_ele)
            ...
            td_elements = soup.find_all('td')
            td_values = [td.get_text(strip=True) for td in td_elements]
            sub_lists = [td_values[i:i + 7]
                         for i in range(0, len(td_values), 7)]
    ...
    def _anotaciones_vigentes(driver: webdriver.Chrome, doc: Document):
        try:
            click_ele = driver.find_element(
                By.XPATH, "//div[@id='comunicaSII5']//div[@class='box_arrow']")
            driver.execute_script("arguments[0].click();", click_ele)
    ...
    def _notificaciones(driver: webdriver.Chrome, doc: Document):
        try:
            click_ele = driver.find_element(
                By.XPATH, "//div[@id='comunica1']//div[@class='box_arrow']")
            driver.execute_script("arguments[0].click();", click_ele)
        ...
        for year in years:
            for month in months:
                try:
                    ...
                    time.sleep(1)
                    driver.find_element(
                        By.XPATH, "//button[normalize-space()='Consultar']").click()
                    table_xpath = "//table[@class='table table-sm ng-scope']//tbody"
                    WebDriverWait(driver, 5).until(
                        EC.presence_of_element_located((By.XPATH, table_xpath)))
                    html = driver.find_elements(By.XPATH, table_xpath)[
                    ...
                    header = driver.find_element(
                        By.XPATH, "//strong[@class='ng-binding']").text
    ...
    def clave_orden(self, fecha_titulo):
        match = re.search(
            r"Observaciones: (\w+) de (\d{4})", fecha_titulo, re.IGNORECASE)
        if match:
    ...
    def _page1(self, driver: webdriver.Chrome, doc):
        ...
        click_ele = driver.find_element(
            By.XPATH, "//a[@id='menu_datos_contribuyente']")
        driver.execute_script("arguments[0].click();", click_ele)
        ...
        click_ele = driver.find_element(
            By.XPATH, "//a[@id='menu_comunicados_sii']")
        driver.execute_script("arguments[0].click();", click_ele)
    ...
    def _page3(self, driver: webdriver.Chrome, doc):
        driver.get(
            'https://www4.sii.cl/sifmConsultaInternet/index.html?form=29&dest=cifxx')
        time.sleep(5)
        try:
            f29 = "//a[normalize-space()='F29 (+)']"
            WebDriverWait(driver, 300).until(
                EC.presence_of_element_located((By.XPATH, f29)))
            ...
            html = driver.find_element(
                By.CSS_SELECTOR, table_css).get_attribute('innerHTML')
            soup = BeautifulSoup(html, 'html.parser')
            td_elements = soup.find_all('td')
            observacion_indices = []
            ...
            tabla_elemento = driver.find_element(
                By.XPATH, '//*[@id="frame-window"]/table/tbody/tr[2]/td/table/tbody/tr/td/div/table[2]/tbody/tr/td[2]/table/tbody/tr[1]/td/table/tbody/tr[1]/td/table/tbody/tr[3]/td/table')
            driver.execute_script(
                "arguments[0].scrollIntoView();", tabla_elemento)
            time.sleep(1)
            tabla_elemento.screenshot("tabla_general.png")

            observaciones = []
            selenium_tds = driver.find_elements(
                By.CSS_SELECTOR, table_css + " td")
            ...
            for idx in observacion_indices:
                try:
                    ...
                    obs_html = driver.find_element(
                        By.CSS_SELECTOR, "table[border='1']").get_attribute('outerHTML')

                    titulo_periodo = driver.find_element(
                        By.CSS_SELECTOR, "h2.gw-h2").text
                    match = re.search(
                        r"Periodo\s+(\w+)\s+de\s+(\d{4})", titulo_periodo)
                    if match:
                        mes = match.group(1)
                        anio = match.group(2)
                    ...
                    WebDriverWait(driver, 30).until(
                        EC.presence_of_element_located(
                            (By.XPATH, "//a[normalize-space()='F29 (+)']"))
                    )
                    driver.find_element(
                        By.XPATH, "//a[normalize-space()='F29 (+)']").click()
                    time.sleep(5)
                    ...
                    logging.warning(
                        f"Error extrayendo observación en idx={idx}: {str(e)}")
                    continue
        ...
        for year in year_value:
            for month in month_value:
                try:
                    codes = []
                    driver.get(
                        'https://www4.sii.cl/rfiInternet/consulta/index.html#rfiSelFormularioPeriodo')
                    driver.refresh()
                    select_box_title_xpath = "//button[normalize-space()='Buscar Datos Ingresados']"
                    WebDriverWait(driver, 30).until(
                        EC.presence_of_element_located((By.XPATH, select_box_title_xpath)))
                    first_dropdown = driver.find_elements(
                        By.XPATH, '//select[@class="gwt-ListBox"]')[0]
                    first_select = Select(first_dropdown)
                    first_select.select_by_value("0")
                    time.sleep(5)
                    second_dropdown = driver.find_elements(
                        By.XPATH, '//select[@class="gwt-ListBox"]')[1]
                    second_select = Select(second_dropdown)
                    second_select.select_by_visible_text(year)
                    time.sleep(3)
                    third_dropdown = driver.find_elements(
                        By.XPATH, '//select[@class="gwt-ListBox"]')[2]
                    third_select = Select(third_dropdown)
                    third_select.select_by_visible_text(month)
                    time.sleep(3)
                    title_ele = driver.find_element(
                        By.XPATH, select_box_title_xpath)
                    ...
                    links_ele = driver.find_elements(
                        By.XPATH, '//div[@class="gwt-Hyperlink"]//a')
                    if len(links_ele) > 2:
                        driver.find_elements(
                            By.XPATH, '//div[@class="gwt-Hyperlink"]//a')[2].click()
                        time.sleep(5)
                        driver.find_element(
                            By.XPATH, "//button[normalize-space()='Ver Datos']").click()
                        code_table_xpath = "//div[@class='gwt-HTML']//div[3]"
                        WebDriverWait(driver, 30).until(
                            EC.presence_of_element_located((By.XPATH, code_table_xpath)))
                        trs = driver.find_elements(
                            By.XPATH, "//div[@class='gwt-HTML']//div//tr")
                        for tr in trs:
                            tr_html = tr.get_attribute('innerHTML')
                            if tr_html:
                                soup = BeautifulSoup(tr_html, 'html.parser')
                                td_elements = soup.find_all('td')
                                td_values = [td.get_text(
                                    strip=True) for td in td_elements]
                                filtered_items = [
                                    item for item in td_values if item not in ['+', '-', '=']]
                        ...
                        logging.info(
                            f"--> FORMULARIO 29 - {month} - {year}' extracted.")
    ...
    def _page5(self, driver: webdriver.Chrome, doc):
        ...
        for year in year_value:
            try:
                driver.get(
                    'https://www4.sii.cl/consultaestadof22ui/#!/default')
                ...
                WebDriverWait(driver, 30).until(
                    EC.presence_of_element_located((By.XPATH, social_path)))
                ...
                driver.find_element(
                    By.XPATH, ver_formulario_22_ompacto_button).click()

                codes = ["1412", "1430", "1729", "36", "1445", "1459",
                         "1484", "1496", "1513", "1564", "91", "87"]
                data = []
                ...
                for item in tables_ele:
                    ...
                    lines.pop(-1)
                    matching_indices = [
                        index for index, value in enumerate(lines) if value in codes]
                    if matching_indices:
                ...
                doc.add_heading(
                    f'CONSULTA DE ESTADO DE DECLARACIÓN DE RENTA {year}', 1)
    ...
    def _page7(self, driver: webdriver.Chrome, doc):
        ...
        for year in years:
            try:
                ...
                td_elements = soup.find_all('td')
                td_values = [td.get_text(strip=True) for td in td_elements]
                sub_lists = [td_values[i:i + 6]
                             for i in range(0, len(td_values), 6)]
    ...
    def _page8(self, driver: webdriver.Chrome, doc):
        ...
        try:
            ...
            WebDriverWait(driver, 30).until(
                EC.presence_of_element_located((By.XPATH, query_btn)))
            ...
            for codigo in codigos:
                try:
                    ...
                    valor = driver.execute_script(
                        "return arguments[0].value;", elemento)
                    if not valor:
                        valor = elemento.text
            ...
            no_restitucion = (v["1279"] + v["1496"]) / \
                factor_no_restitucion if (v["1279"] + v["1496"]) > 0 else 0
            restitucion = (v["1300"] + v["1498"]) / \
                factor_restitucion if (v["1300"] + v["1498"]) > 0 else 0

            # Valores base
            tabla_datos = [
                ["No sujeto a restitución", no_restitucion, str(
                    v["1270"] + v["1495"]), str(v["1279"] + v["1496"]), factor_no_restitucion],
                ["Sujeto a restitución", restitucion, str(
                    v["1497"] + v["1288"]), str(v["1300"] + v["1498"]), factor_restitucion],
                ["STUT", v["1382"] + v["1260"], "", "", ""],
                ["ISFUT", v["1730"] + v["1752"], "", "", ""],
                ["IPE", v["1313"] + v["1449"], "",
                    "RAI", str(v["1210"] + v["1451"])],
                ["RAP", v["1452"] + v["1221"], "", "", ""],
                ["INR", v["1454"] + v["1246"], "", "", ""],
            ...
            # Total
            total = sum(f[1]
                        for f in tabla_datos if isinstance(f[1], (int, float)))
            tabla_datos.append(["Total", total, "", "", ""])

            # Documento
            doc.add_heading(
                f'Detalle de utilidades disponibles para distribución según RTRE al 31-12-{year}', 1)
            ...
            for row in tabla_datos:
                ...
                if isinstance(row[1], (int, float)) and row[1] != 0:
                    cells[1].text = f"{row[1]:,.2f}".replace(
                        ",", "X").replace(".", ",").replace("X", ".")
                else:
                    cells[1].text = ""

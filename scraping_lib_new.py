...
def scrape_data_for_account(rut, clave, year, month, target_codes):
    ...
    try:
        ...
        account_results["RETENCION TERCEROS"] = get_retencion_terceros(driver=driver, month=month, year=year)
...
def get_retencion_terceros(driver, month, year):
        try:
            driver.get('https://zeus.sii.cl/cvc_cgi/bte/bte_indiv_cons?1')
            time.sleep(5)
            month_select = Select(driver.find_element(By.ID, 'MESM'))
            month_select.select_by_visible_text(month)
            time.sleep(1)
            year_select = Select(driver.find_element(By.ID, 'ANOM'))
            year_select.select_by_visible_text(year)
            time.sleep(1)
            driver.find_element(By.NAME, "consultaM").click()
            WebDriverWait(driver, 20).until(EC.presence_of_element_located((By.XPATH, "//table[contains(@width,'920') and contains(@border,'1')]")))
            time.sleep(1)
            table = driver.find_element(By.XPATH, "//table[contains(@width,'920') and contains(@border,'1')]")
            rows = table.find_elements(By.TAG_NAME, "tr")
            total_row = rows[-1]
            cells = total_row.find_elements(By.TAG_NAME, "td")
